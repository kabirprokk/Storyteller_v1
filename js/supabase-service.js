(function () {
  ...
  const normalize = row => ({
    ...
    authorId: row.author_id, username: row.author_username || row.profiles?.username, authorAvatar: row.author_avatar_url || row.profiles?.avatar_url || '', authorDonationQr: row.author_donation_qr_url || row.profiles?.donation_qr_url || '', authorContactEmail: row.author_contact_email || row.profiles?.contact_email || '', authorContactSource: row.author_contact_source || row.profiles?.contact_source || '', authorInstagram: row.author_instagram_handle || row.profiles?.instagram_handle || '', cat: row.category_name || row.categories?.name || 'Uncategorized',
  ...
  window.StoryAPI = {
    ...
    async becomeWriter(contactEmail,contactSource,instagramHandle='') { const session=await this.session();if(!session)throw new Error('Sign in first');const {data,error}=await required().rpc('become_writer',{writer_email:String(contactEmail||'').trim().toLowerCase(),writer_source:String(contactSource||'').trim(),writer_instagram:String(instagramHandle||'').trim().replace(/^@/,'')});if(error)throw error;return data; },
    ...
    async uploadWriterDonationQr(file) { const session=await this.session();if(!session)throw new Error('Sign in first');if(!['image/png','image/jpeg','image/webp'].includes(file.type))throw new Error('Use a PNG, JPEG, or WebP QR image');if(file.size>3*1024*1024)throw new Error('QR image must be under 3 MB');const ext=file.type==='image/png'?'png':file.type==='image/webp'?'webp':'jpg';const path=`${session.user.id}/writer-payment-qr-${Date.now()}.${ext}`;const client=required();const upload=await client.storage.from('donation-assets').upload(path,file,{cacheControl:'3600',upsert:false});if(upload.error)throw upload.error;const donation_qr_url=client.storage.from('donation-assets').getPublicUrl(path).data.publicUrl;const update=await client.from('profiles').update({donation_qr_url}).eq('id',session.user.id);if(update.error)throw update.error;return donation_qr_url; },
    async clearWriterDonationQr() { const session=await this.session();if(!session)throw new Error('Sign in first');const {error}=await required().from('profiles').update({donation_qr_url:null}).eq('id',session.user.id);if(error)throw error; },
    ...
    async view(storyId) { if(await isHardened()){const {data,error}=await required().functions.invoke('record-story-view',{body:{storyId}});if(error)throw error;return Boolean(data?.counted)}await required().rpc('increment_story_view',{target:storyId});return true; },
