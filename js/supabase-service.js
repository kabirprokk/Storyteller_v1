(function () {
  ...
  window.StoryAPI = {
    ...
    async becomeWriter(contactEmail,contactSource) { const session=await this.session();if(!session)throw new Error('Sign in first');const {data,error}=await required().rpc('become_writer',{writer_email:String(contactEmail||'').trim().toLowerCase(),writer_source:String(contactSource||'').trim()});if(error)throw error;return data; },
